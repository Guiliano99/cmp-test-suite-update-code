...
def _validate_composite_sig_conform_to_spki(
    ...
    name = COMPOSITE_SIG_OID_TO_NAME[prot_alg_id["algorithm"]]

    if cert_alg_id["algorithm"] not in COMPOSITE_SIG_OID_TO_NAME:
        return False

    if "rsa" in name:
        name_spki = COMPOSITE_SIG_OID_TO_NAME[cert_alg_id["algorithm"]]
...
def check_protection_alg_conform_to_spki(
    ...
    if prot_alg_id["algorithm"] in COMPOSITE_SIG_OID_TO_NAME:
