...
"""Contains functionality which is only needed to test a client CMP-implementation."""
...
def validate_cert_template_public_key(
    ...
    if isinstance(public_key, RSAPublicKey):
        if public_key.key_size < 2048:
            raise BadCertTemplate("The RSA public key was shorter than 2048 bits")

        if max_key_size is not None and public_key.key_size > max_key_size:
            raise BadCertTemplate(f"The RSA public key was longer than {max_key_size} bits")
...
def build_pki_conf_from_cert_conf(  # noqa: D417 Missing argument descriptions in the docstring
    ...
    **kwargs,
    ...
    Ensures that the client correctly received the certificates.
...
def _process_crr_single(
    ...
    if result:
        raise BadRequest(
            "The `CCR` message can not be for a `KGA` request. "
            "The private key must be securely generated by the client."
        )
