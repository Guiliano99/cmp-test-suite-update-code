...
CA MUST Reject Protected Genm With Get CA Certs
...
CA MUST Respond To Protected Genm With Get CA Certs With Invalid InfoValue
    [Documentation]    According to RFC 9483 Section 4.3.1, an End-Entity can request the CA's certificate chain using
...
CA MUST Respond To Protected Genm With Get Root CA Certificate Update
...
CA MUST Reject Protected Genm With Get Root CA Cert Update Without OldRootCert
    [Documentation]    According to RFC 9483 Section 4.3.2, an End-Entity requesting a root CA certificate update
    ...    using a protected general message must include the old root certificate for verification.
...
CA MUST Respond Protected Genm With Get Certificate Request Template
...
CA MUST Accept Protected Genm With Get Cert Template With CertProfile Set
    [Documentation]    According to RFC 9483 Section 4.3.3, an End-Entity can request a certificate request template
    ...    using a general message with the `id-it-certReqTemplate` InfoType. This allows the End-Entity
    ...    to understand which values it can set. We send a protected general message with a
    ...
    ${protected_genm}=    Default Protect PKIMessage    ${patched_genm}
    ${genp}=    Exchange PKIMessage    ${protected_genm}
    Validate Get Certificate Request Template    ${genp}

CA MUST Reject Protected Genm With Get Cert Template With Invalid InfoValue
    [Documentation]    According to RFC 9483 Section 4.3.3, an End-Entity can request a certificate request template
...
CA MUST Respond To Valid Protected CurrentCRL Request
...
CA MUST Reject Invalid Value For CurrentCRL Request
    [Documentation]    According to RFC 9483 Section 4.3.4, an End-Entity can request the latest available
...
CA MUST Respond To Valid Protected CRL Update Retrieval
    [Documentation]    According to RFC 9483 Section 4.3.4, an End-Entity can request a CRL update using a
    ...    protected general message with the `id-it-crlStatusList` InfoType. The request MUST include
    ...    a CRL source, either a CRL distribution point name or issuer name. We send a protected
...
CA MUST Respond To Valid Protected CRL Update Retrieval With CRL File
    [Documentation]    According to RFC 9483 Section 4.3.4, an End-Entity can request a CRL update using a
    ...    protected general message with the `id-it-crlUpdateRet` InfoType. We send a protected
