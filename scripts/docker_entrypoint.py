...
""",
    ...
    parser.add_argument(
        "--profile",
        help=(
            "CMP profile to test against (e.g. lwcmp, gppcmp). "
            "Sets protocol field sizes and validation rules. Overrides the CMP_PROFILE variable."
        ),
        type=str,
        default=None,
        metavar="NAME",
...
├── data/          -  a directory with data files (keys, certificates, etc.) needed by your custom.robot"""
    ...
    # the end.
    if args.profile:
        command += f" --variable CMP_PROFILE:{args.profile}"
